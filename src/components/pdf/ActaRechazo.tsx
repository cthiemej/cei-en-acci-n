...
export const ActaRechazo = ({ code, title, investigator, faculty, sessionNumber, sessionDate, voteResult, resolutionSummary, generatedDate }: Props) => (
  <Document>
    <Page size="A4" style={styles.page}>
      <PDFHeader />
      <Text style={styles.title}>Acta de Rechazo</Text>
      ...
      <View style={{ marginTop: 30 }}>
        <Text style={styles.body}>
          El Comité de Ética en Investigación de la Universidad Diego Portales, en su Sesión {sessionNumber} de fecha {sessionDate}, ha resuelto RECHAZAR éticamente el proyecto de investigación {code} titulado "{title}", presentado por {investigator}.
        </Text>
        {resolutionSummary && (
          <>
            <Text style={[styles.label, { marginTop: 12 }]}>Fundamentos del rechazo:</Text>
            <Text style={styles.body}>{resolutionSummary}</Text>
          </>
        )}
      </View>
